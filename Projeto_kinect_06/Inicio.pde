...
void setupInicio(){
  ...
  imgDifferencesStart = createImage(width, height, ARGB);
...
void calculateOverlapStart(){
  ...
  for (int i = 0; i < poseStart.pixels.length; i++) {
    ...
    if (diffRStart == 0 && diffGStart == 0 && diffBStart == 0) {
  imgDifferencesStart.pixels[i] = color(0, 0, 0, 0);  // Transparent black
} else {
  imgDifferencesStart.pixels[i] = color(diffRStart, diffGStart, diffBStart, 255);  // Opaque
}
...
void timerStart(){
  ...
  if (elapsedTimeInPoseStart / 1000 > 5) {playSelect(); setupMenu(); scene = 1;}
}
